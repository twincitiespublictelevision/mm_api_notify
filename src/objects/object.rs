...
impl Object {
    ...
    pub fn as_document(&self) -> IngestResult<Document> {
        if let bson::Bson::Document(document) = self.as_bson() {
            Ok(document)
        } else {
    ...
    fn as_bson(&self) -> Bson {
        utils::map_string_to_bson_dates(self.as_json().into())
    }
    ...
    fn import_parents<T: StorageEngine, S: ThreadedAPI>(
        ...
        parent_types
            ...
            .map(|parent_type| match self.attributes.get(parent_type) {
                Some(parent_obj) => Ref::from_json(parent_obj)
...
mod tests {
    ...
    fn gets_parent_from_store() {
        ...
        let parent_obj =
            Object::from_json(&serde_json::from_str(parent_obj_resp).unwrap()).unwrap();
