...
impl Object {
    ...
    pub fn parent<T: StorageEngine>(&self, store: &T) -> Option<Object> {
        ...
        match self.object_type.as_str() {
            ...
            self.attributes.get(parent_key).and_then(|parent| {
                Ref::from_json(parent).ok()
            })
        })
            .and_then(|parent_ref| {
                store
    ...
    fn import_children<T: StorageEngine, S: ThreadedAPI>(
        &self,
        runtime: &Runtime<T, S>,
        follow_refs: bool,
        since: i64,
    ...
    fn import_parents<T: StorageEngine, S: ThreadedAPI>(
        &self,
        runtime: &Runtime<T, S>,
        _: bool,
        since: i64,
...
impl Importable for Object {
    fn import<T: StorageEngine, S: ThreadedAPI>(
        ...
        let mut update_result = if updated_at_time >= since {
            ...
            if res.is_ok() && runtime.config.enable_hooks && runtime.config.hooks.is_some() {
                Payload::from_object(self, &runtime.store)
                    .and_then(|payload| {
                        runtime.config.hooks.as_ref().map(|hooks| {
                            payload.emitter(&hooks, HttpEmitter::new).update()
                        })
                    })
                    .or_else(|| {
        ...
        if follow_refs {
            ...
            update_result = (
                update_result.0 + child_results.0,
                update_result.1 + child_results.1,
            );

            let parent_results = self.import_parents(runtime, follow_refs, since);
            update_result = (
                update_result.0 + parent_results.0,
    ...
    fn from_json(json: &Json) -> IngestResult<Object> {
        ...
        let id = source.get("data").and_then(|data| {
            data.get("id").and_then(|id| {
                id.as_str().and_then(|id_str| Some(id_str.to_string()))
            })
        });

        let obj_type = source.get("data").and_then(|data| {
            data.get("type").and_then(|o_type| {
                o_type.as_str().and_then(
        ...
        source
            ...
            .and_then(|map| {
                ...
                let attrs = map.remove("data").and_then(|mut data| {
                    data.as_object_mut().and_then(
                        |data_map| data_map.remove("attributes"),
                    )
                });

                let self_url = map.remove("links").and_then(|mut data| {
                    data.as_object_mut().and_then(|data_map| {
                        data_map.remove("self").unwrap().as_str().map(
                            |s| s.to_string(),
                        )
                    })
                });
...
mod tests {
    ...
    fn emits_update_if_new() {
        ...
        runtime.store.set_response(
            Object::from_json(&obj_json).unwrap(),
        );
