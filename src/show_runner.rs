...
pub fn run(api: &ThreadedAPI, db: &Database, run_start_time: i64) -> IngestResult<Duration> {
    ...
    match number_of_pages {
        Ok(count) => {
            ...
            let response = api.shows(vec![]);

            let show_coll = match response {
                Ok(response_string) => {
                    let full_json: JsonResult<Json> =
                        serde_json::from_str(response_string.as_str());
                    match full_json {
            ...
            show_coll.unwrap().import(api, db, true, &vec![], run_start_time);
...
pub fn run_show(show_identifier: &str,
    ...
    match show {
        Ok(show) => show.import(&api, &db, true, &vec![], run_start_time),
        Err(_) => (),
    };
        ...
        Ok(show_objects) => {
            ...
            for show in show_objects {
                ...
                show_pool.add_worker(thread::spawn(move || {
                    show.import(&shared_api, &shared_db, true, &vec![], run_start_time);
                }));
            }
